...
class KmlContainerParser {
    ...
    private final static String PROPERTY_REGEX = "name|description|visibility|open|address|" +
            "phoneNumber";
    ...
    private final static String UNSUPPORTED_REGEX = "altitude|altitudeModeGroup|altitudeMode|" +
            ...
            "gridOrigin|httpQuery|leftFov|linkDescription|linkName|linkSnippet|listItemType|" +
            "maxSnippetLines|maxSessionLength|message|minAltitude|minFadeExtent|minLodPixels|" +
            "minRefreshPeriod|maxAltitude|maxFadeExtent|maxLodPixels|maxHeight|maxWidth|near|" +
            "overlayXY|range|refreshMode|refreshInterval|refreshVisibility|rightFov|roll|" +
            "rotationXY|screenXY|shape|sourceHref|state|targetHref|tessellate|tileSize|topFov|" +
            "viewBoundScale|viewFormat|viewRefreshMode|viewRefreshTime|when";
    ...
    private static KmlContainer assignPropertiesToContainer(XmlPullParser parser)
            ...
            if (eventType == START_TAG) {
                if (parser.getName().matches(UNSUPPORTED_REGEX)) {
                    ...
                    setContainerPlacemark(
                            parser, (HashMap<KmlPlacemark, Object>) containerPlacemarks);
        ...
        return new KmlContainer(containerProperties, containerStyles,
                (HashMap<KmlPlacemark, Object>) containerPlacemarks, containerStyleMaps,
                nestedContainers, containerGroundOverlays, containerId);
    }
    ...
    private static void setExtendedDataProperties(
            XmlPullParser parser,
            HashMap<String, String> containerProperties)
            throws XmlPullParserException, IOException {
        ...
        while (!(eventType == END_TAG && parser.getName().equals(EXTENDED_DATA))) {
            if (eventType == START_TAG) {
                if (parser.getName().equals("Data")) {
                    ...
                    containerProperties.put(propertyKey, parser.nextText());
                    propertyKey = null;
                }
            }
            eventType = parser.next();
    ...
    private static void setContainerStyle(
            XmlPullParser parser,
            HashMap<String, KmlStyle> containerStyles)
            throws XmlPullParserException, IOException {
        if (parser.getAttributeValue(null, "id") != null) {
     ...
     */
    private static void setContainerPlacemark(
            XmlPullParser parser,
