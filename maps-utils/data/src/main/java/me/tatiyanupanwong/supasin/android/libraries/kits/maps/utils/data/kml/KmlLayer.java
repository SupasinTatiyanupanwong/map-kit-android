...
public class KmlLayer extends Layer {
    ...
    /**
     * Creates a new KmlLayer object - addLayerToMap() must be called to trigger rendering onto a
     * map.
     * </p>
     ...
     */
    public KmlLayer(MapClient map, int resourceId, Context context)
            throws XmlPullParserException, IOException {
        this(map, context.getResources().openRawResource(resourceId), context,
                new MarkerManager(map), new PolygonManager(map), new PolylineManager(
                        map),
                new GroundOverlayManager(map), null);
    }
     ...
     * @throws IOException if I/O error
     */
    public KmlLayer(MapClient map, InputStream stream, Context context)
            throws XmlPullParserException, IOException {
        this(map, stream, context, new MarkerManager(map), new PolygonManager(map),
                new PolylineManager(map), new GroundOverlayManager(map), null);
    }
    ...
    public KmlLayer(
            ...
            throws XmlPullParserException, IOException {
        this(map, context.getResources().openRawResource(resourceId), context, markerManager,
                polygonManager, polylineManager, groundOverlayManager, cache);
    }
    ...
    public KmlLayer(
        ...
        KmlRenderer renderer = new KmlRenderer(map, context, markerManager, polygonManager,
                polylineManager, groundOverlayManager, cache);
        ...
        try {
            ...
            if (entry != null) { // is a KMZ zip file
                ...
                while (entry != null) {
                    if (parser == null && entry.getName().toLowerCase().endsWith(".kml")) {
                        ...
                        if (bitmap != null) {
                            ...
                            Log.w("KmlLayer", "Unsupported KMZ contents file type: " +
                                    entry.getName());
                        }
                    }
                ...
                renderer.storeKmzData(parser.getStyles(), parser.getStyleMaps(),
                        parser.getPlacemarks(), parser.getContainers(), parser.getGroundOverlays(),
                        images);
            } else { // is a KML
                bis.reset();
                parser = parseKml(bis);
                renderer.storeKmlData(parser.getStyles(), parser.getStyleMaps(),
                        parser.getPlacemarks(), parser.getContainers(), parser.getGroundOverlays());
            }
