...
class KmlFeatureParser {
    ...
    private final static String GEOMETRY_REGEX = "Point|LineString|Polygon|MultiGeometry|Track|" +
            "MultiTrack";
    ...
    private final static String PROPERTY_REGEX = "name|description|drawOrder|visibility|open|" +
            "address|phoneNumber";
    ...
    private static LatLngAlt convertToLatLngAlt(String coordinateString, String separator) {
        ...
        Double alt =
                (coordinate.length > 2) ? Double.parseDouble(coordinate[ALTITUDE_INDEX]) : null;
        LatLng latLng = MapKit.newLatLng(lat, lon);
    ...
    private static LatLngBounds createLatLngBounds(Double north, Double south, Double east,
            Double west) {
        LatLng southWest = MapKit.newLatLng(south, west);
        LatLng northEast = MapKit.newLatLng(north, east);
