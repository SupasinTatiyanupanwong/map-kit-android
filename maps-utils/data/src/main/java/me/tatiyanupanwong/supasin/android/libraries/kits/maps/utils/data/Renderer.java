...
import me.tatiyanupanwong.supasin.android.libraries.kits.maps.utils.data.geojson.BiMultiMap;
...
import me.tatiyanupanwong.supasin.android.libraries.kits.maps.utils.data.kml.KmlMultiGeometry;
import me.tatiyanupanwong.supasin.android.libraries.kits.maps.utils.data.kml.KmlPlacemark;
import me.tatiyanupanwong.supasin.android.libraries.kits.maps.utils.data.kml.KmlPoint;
import me.tatiyanupanwong.supasin.android.libraries.kits.maps.utils.data.kml.KmlRenderer;
...
public class Renderer {
    ...
    public Renderer(
        ...
        this(map, new HashSet<String>(), null, null, null, new BiMultiMap<Feature>(), markerManager,
                polygonManager, polylineManager, groundOverlayManager);
        mContext = context;
    ...
    public Renderer(
            MapClient map,
            HashMap<? extends Feature, Object> features,
            MarkerManager markerManager,
            PolygonManager polygonManager,
            PolylineManager polylineManager,
            GroundOverlayManager groundOverlayManager) {
    ...
    public static final class ImagesCache {
         ...
         * This cache is cleared once all icon URLs are loaded, scaled, and cached as
         * BitmapDescriptors.
         */
     ...
     *
     * @return {@code true} if layer is on map, {@code false} otherwise
     */
     ...
     *
     * @param layerOnMap contains {@code true} if the layer should be set to visible and
    ...
    private BitmapDescriptor scaleIcon(Bitmap unscaledBitmap, double scale) {
        ...
        return MapKit.getBitmapDescriptorFactory().fromBitmap(scaledBitmap);
    }
    ...
    protected BitmapDescriptor getCachedGroundOverlayImage(String url) {
        ...
        if (bitmapDescriptor == null) {
            ...
            if (bitmap != null) {
                bitmapDescriptor =
                        MapKit.getBitmapDescriptorFactory().fromBitmap(bitmap);
    ...
    protected void checkClearBitmapCache() {
        if (mNumActiveDownloads == 0
                && mImagesCache != null
                && !mImagesCache.bitmapCache.isEmpty()) {
        ...
        if (mLayerOnMap) {
            ...
            if (feature.hasGeometry()) {
                ...
                if (feature instanceof KmlPlacemark) {
                    ...
                    mapObject = addKmlPlacemarkToMap((KmlPlacemark) feature, geometry, style,
                            inlineStyle, isPlacemarkVisible);
                } else {
    ...
    protected Object addGeoJsonFeatureToMap(Feature feature, Geometry geometry) {
        ...
        switch (geometryType) {
            case "Point":
                Marker.Options markerOptions = null;
                if (feature instanceof GeoJsonFeature) {
            ...
            case "LineString":
                Polyline.Options polylineOptions = null;
                if (feature instanceof GeoJsonFeature) {
            ...
            case "Polygon":
                Polygon.Options polygonOptions = null;
                if (feature instanceof GeoJsonFeature) {
    ...
    protected Object addKmlPlacemarkToMap(KmlPlacemark placemark, Geometry geometry, KmlStyle style,
        ...
        switch (geometryType) {
            case "Point":
                Marker.Options markerOptions = style.getMarkerOptions();
                if (inlineStyle != null) {
                ...
                return marker;
            case "LineString":
                Polyline.Options polylineOptions = style.getPolylineOptions();
                if (inlineStyle != null) {
                ...
                return polyline;
            case "Polygon":
                Polygon.Options polygonOptions = style.getPolygonOptions();
                if (inlineStyle != null) {
                ...
                return polygon;
            case "MultiGeometry":
                return addMultiGeometryToMap(placemark, (KmlMultiGeometry) geometry, style,
                        inlineStyle, isVisible);
        }
        return null;
    }
     ...
     */
    private Marker addPointToMap(Marker.Options markerOptions, Point point) {
     ...
     */
    private void setInlinePointStyle(Marker.Options markerOptions, KmlStyle inlineStyle,
                                     KmlStyle defaultStyle) {
        Marker.Options inlineMarkerOptions = inlineStyle.getMarkerOptions();
     ...
     */
    private Polyline addLineStringToMap(Polyline.Options polylineOptions,
     ...
     * @param inlineStyle     inline styles to apply
     */
    private void setInlineLineStringStyle(Polyline.Options polylineOptions, KmlStyle inlineStyle) {
        Polyline.Options inlinePolylineOptions = inlineStyle.getPolylineOptions();
     ...
     */
    private Polygon addPolygonToMap(Polygon.Options polygonOptions, DataPolygon polygon) {
     ...
     * @param inlineStyle    inline styles to apply
     */
    private void setInlinePolygonStyle(Polygon.Options polygonOptions, KmlStyle inlineStyle) {
        Polygon.Options inlinePolygonOptions = inlineStyle.getPolygonOptions();
     ...
     * @return {@code false} if a Feature has a visibility value of "1", {@code true} otherwise.
     */
     ...
     */
    private ArrayList<Object> addMultiGeometryToMap(
    ...
    private ArrayList<Polyline> addMultiLineStringToMap(GeoJsonLineStringStyle lineStringStyle,
        ...
        for (GeoJsonLineString geoJsonLineString : multiLineString.getLineStrings()) {
            polylines.add(
                    addLineStringToMap(lineStringStyle.toPolylineOptions(), geoJsonLineString));
        }
        return polylines;
    }
     ...
     */
    private void addMarkerIcons(String styleUrl, double scale, Marker.Options markerOptions) {
     ...
     */
    protected GroundOverlay attachGroundOverlay(GroundOverlay.Options groundOverlayOptions) {
    ...
    private void setMarkerInfoWindow(KmlStyle style, Marker marker,
        ...
        if (hasBalloonOptions && hasBalloonText) {
            marker.setTitle(KmlUtils.substituteProperties(
                    style.getBalloonOptions().get("text"), placemark));
            createInfoWindow();
    ...
    private void createInfoWindow() {
        mMarkers.setInfoWindowAdapter(new MapClient.InfoWindowAdapter() {
            public View getInfoWindow(@NonNull Marker marker) {
                return null;
            }

            public View getInfoContents(@NonNull Marker marker) {
                View view = LayoutInflater.from(mContext).inflate(R.layout.mku_info_window, null);
                TextView infoWindowText = view.findViewById(R.id.window);
                if (marker.getSnippet() != null) {
                    infoWindowText.setText(
                            Html.fromHtml(marker.getTitle() + "<br>" + marker.getSnippet()));
                } else {
                    infoWindowText.setText(
                            Html.fromHtml(marker.getTitle()));
                }
                return view;
            }
        });
    ...
    void setOnFeatureClickListener(final Layer.OnFeatureClickListener listener) {
        ...
        mPolygons.setOnPolygonClickListener(new MapClient.OnPolygonClickListener() {
            @Override
        ...
        mMarkers.setOnMarkerClickListener(new MapClient.OnMarkerClickListener() {
            @Override
        ...
        mPolylines.setOnPolylineClickListener(new MapClient.OnPolylineClickListener() {
            @Override
