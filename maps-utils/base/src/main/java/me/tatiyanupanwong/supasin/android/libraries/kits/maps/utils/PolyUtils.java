...
package me.tatiyanupanwong.supasin.android.libraries.kits.maps.utils;
...
import static me.tatiyanupanwong.supasin.android.libraries.kits.maps.utils.MathUtils.EARTH_RADIUS;
import static me.tatiyanupanwong.supasin.android.libraries.kits.maps.utils.MathUtils.clamp;
import static me.tatiyanupanwong.supasin.android.libraries.kits.maps.utils.MathUtils.hav;
import static me.tatiyanupanwong.supasin.android.libraries.kits.maps.utils.MathUtils.havDistance;
import static me.tatiyanupanwong.supasin.android.libraries.kits.maps.utils.MathUtils.havFromSin;
import static me.tatiyanupanwong.supasin.android.libraries.kits.maps.utils.MathUtils.inverseMercator;
import static me.tatiyanupanwong.supasin.android.libraries.kits.maps.utils.MathUtils.mercator;
import static me.tatiyanupanwong.supasin.android.libraries.kits.maps.utils.MathUtils.sinFromHav;
import static me.tatiyanupanwong.supasin.android.libraries.kits.maps.utils.MathUtils.sinSumFromHav;
import static me.tatiyanupanwong.supasin.android.libraries.kits.maps.utils.MathUtils.wrap;
import static me.tatiyanupanwong.supasin.android.libraries.kits.maps.utils.SphericalUtils.computeDistanceBetween;
...
public class PolyUtils {
    ...
    public static boolean containsLocation(
            double latitude, double longitude, List<LatLng> polygon, boolean geodesic) {
    ...
    /**
     * Same as {@link #isLocationOnPath(LatLng, List, boolean, double)} with a default tolerance
     * of 0.1 meters.
     */
    public static boolean isLocationOnPath(LatLng point, List<LatLng> polyline,
                                           boolean geodesic) {
    ...
    /**
     * Computes whether (and where) a given point lies on or near a polyline, within a specified
     * tolerance.
     * The polyline is not closed -- the closing segment between the first point and the last point
     * is not included.
     *
    ...
    /**
     * Same as {@link #locationIndexOnPath(LatLng, List, boolean, double)} with a default tolerance
     * of 0.1 meters.
     */
     ...
     * Computes whether (and where) a given point lies on or near a polyline, within a specified
     * tolerance.
     * If closed, the closing segment between the last and first points of the polyline is not
     * considered.
     *
     * @param point          our needle
     * @param poly           our haystack
     ...
     * poly.size()-2 if between poly[poly.size() - 2] and poly[poly.size() - 1]
     */
    public static int locationIndexOnEdgeOrPath(
            LatLng point,
            List<LatLng> poly,
            boolean closed,
            boolean geodesic,
    ...
    public static List<LatLng> simplify(List<LatLng> poly, double tolerance) {
        ...
        if (closedPolygon) {
            ...
            // LatLng.getLatitude() and .getLongitude() are immutable, so replace the last point
            poly.remove(poly.size() - 1);
            poly.add(MapKit.newLatLng(
                    lastPoint.getLatitude() + OFFSET, lastPoint.getLongitude() + OFFSET));
        }
     ...
     * @return {@code true} if the provided list of points is a closed polygon (i.e., the first and last
     * points are the same), and {@code false} if it is not
     */
    ...
    public static double distanceToLine(final LatLng p, final LatLng start, final LatLng end) {
        ...
        LatLng su = MapKit.newLatLng(
                start.getLatitude() + u * (end.getLatitude() - start.getLatitude()),
                start.getLongitude() + u * (end.getLongitude() - start.getLongitude()));
        return computeDistanceBetween(p, su);
    }

    /**
     * Decodes an encoded path string into a sequence of {@link LatLng}s.
     */
    public static List<LatLng> decode(final String encodedPath) {
        ...
        while (index < len) {
            ...
            path.add(MapKit.newLatLng(lat * 1e-5, lng * 1e-5));
        }
    ...
    /**
     * Encodes a sequence of {@link LatLng}s into an encoded path string.
     */
